...
同一会话中多次调用技能时，避免重复执行项目级别的环境检测和配置读取。

#### 核心原则

- **项目静态信息**：首次检测后记住，后续复用。包括但不限于：平台/服务地址、认证状态、项目配置、模板/规范等
- **动态信息**：每次都需重新获取。包括但不限于：当前分支、工作区状态、远程最新数据等

#### 实现方式

涉及缓存的步骤，开头加跳过判断，末尾加缓存提示：

```
**如果对话中已确定过 [XXX]，直接复用，跳过本步骤。**
...
如果信息可被多个技能共享，判断条件写"任意相关技能"；如果仅本技能内部复用，写"本技能"。
